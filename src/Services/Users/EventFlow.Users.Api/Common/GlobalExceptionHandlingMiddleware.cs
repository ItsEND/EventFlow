...
public class GlobalExceptionHandlingMiddleware
{
    ...
    private static (int statusCode, string title, string detail) MapException(Exception ex) =>
        ex switch
        {
            ...
            UserException { Code: UserErrorCode.InvalidCredentials } UserException => (StatusCodes.Status401Unauthorized, "Неверные учетные данные", UserException.Message),
            UserException { Code: UserErrorCode.LoginAlreadyExists } UserException => (StatusCodes.Status409Conflict, "Конфликт", UserException.Message),
            UserException { Code: UserErrorCode.Forbidden } UserException => (StatusCodes.Status403Forbidden, "Доступ запрещён", UserException.Message),
